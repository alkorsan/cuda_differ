...
OPTS_META = [
    ...
    {'opt': 'differ.use_patience_diff',
     'cmt': _('Use patience diff algorithm (better similarity detection, requires patiencediff package)'),
     'def':  True,
     'frm': 'bool',
     'chp': 'config',
     },
...
class Command:
    ...
    def _refresh_ex(self, ed, show_dialog=False):
        ...
        self.diff.set_seqs(a_text_all.splitlines(False),
                           b_text_all.splitlines(False))
        ...
        self.diff.use_patience = self.cfg.get('use_patience_diff', True)

        ct.app_idle(True)

        # Build per-line visual row count maps.
        __, cell_h = a_ed.get_prop(ct.PROP_CELL_SIZE)
        a_wraps = {}
        try:
            for item in a_ed.get_wrapinfo():
                ln = item.get('line', 0)
                a_wraps[ln] = a_wraps.get(ln, 0) + 1
        except Exception:
            pass
        b_wraps = {}
        try:
            for item in b_ed.get_wrapinfo():
                ln = item.get('line', 0)
                b_wraps[ln] = b_wraps.get(ln, 0) + 1
        except Exception:
            pass

        # Use differ.py's compare() for BOTH gaps and markers.
        # _fancy_replace does smart pairing (finds best-matching lines
        # within replace blocks), like WinMerge. With autojunk=False
        # in differ.py, this works correctly for large files too.
        diff_list = list(self.diff.compare())

        gap_ops = []
        pending_a_change = None

        for d in diff_list:
            diff_id = d[0]

            if diff_id == df.A_GAP:
                a_line, n, b_line_start = d[1], d[2], d[3]
                b_height = sum(b_wraps.get(b_line_start + k, 1) for k in range(n))
                gap_ops.append((a_ed, a_line - 1, b_height * cell_h))

            elif diff_id == df.B_GAP:
                b_line, n, a_line_start = d[1], d[2], d[3]
                a_height = sum(a_wraps.get(a_line_start + k, 1) for k in range(n))
                gap_ops.append((b_ed, b_line - 1, a_height * cell_h))

            elif diff_id == df.A_LINE_CHANGE:
                pending_a_change = d[1]

            elif diff_id == df.B_LINE_CHANGE:
                if pending_a_change is not None:
                    a_line = pending_a_change
                    b_line = d[1]
                    a_h = a_wraps.get(a_line, 1)
                    b_h = b_wraps.get(b_line, 1)
                    if a_h > b_h:
                        gap_ops.append((b_ed, b_line, (a_h - b_h) * cell_h))
                    elif b_h > a_h:
                        gap_ops.append((a_ed, a_line, (b_h - a_h) * cell_h))
                    pending_a_change = None

        # Add all gaps.
        gap_count = 0
        for target_ed, after_line, h_size in gap_ops:
            if h_size > 0 and after_line >= -1:
                target_ed.gap(ct.GAP_ADD, after_line, 0,
                              tag=DIFF_TAG, size=h_size,
                              color=self.cfg.get('color_gaps'))
                gap_count += 1

        # Markers — use the SAME diff_list so markers and gaps agree.
        for d in diff_list:
            diff_id, y = d[0], d[1]
    ...
    def calc_gap_size(self, opposite_ed, row, n=1, side='?'):
        """Calculate the pixel height needed for a gap of n lines starting
        at 'row' on the opposite editor. Uses get_wrapinfo() to count the
        actual number of visual rows (including word-wrap) for each line,
        then multiplies by the cell height.

        Unlike CONVERT_CARET_TO_PIXELS (which may return wrong values for
        off-screen lines), get_wrapinfo() calculates wrap info for the
        entire document and is always accurate."""
        __, cell_h = opposite_ed.get_prop(ct.PROP_CELL_SIZE)
        try:
            # Get wrap info for lines row..row+n-1 (inclusive).
            # get_wrapinfo(param1, param2) returns info for lines in [param1, param2].
            wi = opposite_ed.get_wrapinfo(row, row + n - 1)
            if wi:
                # Count the total number of visual rows.
                # Each item in wi represents one visual row.
                visual_rows = len(wi)
                h_size = visual_rows * cell_h
            else:
                # Fallback: no wrap info, assume 1 row per line.
                h_size = cell_h * n
        except Exception as ex:
            msg('calc_gap_size get_wrapinfo failed: {}'.format(ex), level=1)
            h_size = cell_h * n
    ...
    def get_config():
        ...
        config = {
            ...
            'use_patience_diff':
                get_opt('use_patience_diff', True),
        }
